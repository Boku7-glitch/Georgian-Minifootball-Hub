...
import { standings, playerStats, staff } from '@/data/mock'
...
function TeamDetailComponent() {
  ...
  const team = standings.find(
    (s) => s.club.toLowerCase().replace(/\s+/g, "-") === teamId
  )
  ...
  const teamPlayers = playerStats.filter(
    (p) => p.team.toLowerCase() === team.club.toLowerCase()
  )

  const teamStaff = staff.filter(
    (s) => s.team.toLowerCase() === team.club.toLowerCase()
  )
            ...
            <table className="w-full text-left border-collapse">
              ...
              {teamPlayers.length > 0 ? (
                teamPlayers.map((player) => (
                  <tr key={player.id} className="hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="p-4 text-sm font-medium text-muted-foreground text-center">{player.goals}</td>
